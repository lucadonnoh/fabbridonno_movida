...
public class TabellaHashAperta<T, K extends Comparable<K>> implements DizionarioFilm<T, K> {
    ...
    /**
     * Modifica la dimensione della tabella: se è piena la raddoppia, se ci sono
     * meno della metà degli elementi la dimezza.
     *
    ...
    // Dato che la funzione deve valere sia per i dizionari con 1 solo elemento sia
    // per quelli con un'ArrayList come elemento
    // Non facciamo l'ispezione perchè si otterrebbe comunque una complessità
    // lineare
    public void deleteEl(Movie m) {
        ...
        //Controllo messo fuori dal for poichè è meglio prima far tutte le delete poi
        //fare il reshape se no potrebbero avvenire più reshape nello stesso deleteEl
    ...
    public Record<T, K> searchRecord(K k) {
        ...
        while (i < carico)// Qui è corretto tenere i<carico in quanto non serve far piu ispezioni di
                          // quanti ne abbiamo in tabella
        {
            ...
            // Se in una ricerca trovi un deleted non si stoppa ma continua,
            //questo perchè ci potrebbero essere casi di hash uguali e quindi continua a cercare
            if (array[h] != DELETED) {
